...
?>
<?php
 if (isset ($_POST["stylesheet"]) && isset ($_POST["generate"]) &&($_POST["stylesheet"] != "nostylesheet") && ($_POST["generate"] != "nogenerator")) {
	 //echo $_POST["stylesheet"];
	 
	 $result = mysql_query("SELECT Waiter.WFirstName, Waiter.WLastName, Table.TID, Table.TSmoker, Burger.BName, SideDishes.SName, Serve.STimeStamp FROM Waiter, `Table`, Burger, SideDishes, Serve WHERE Waiter.WID = Serve.WID AND Table.TID = Serve.TID AND Burger.BID = Serve.BID AND SideDishes.SID = Serve.SID") or die(mysql_error());
...
<form id="form" name="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
  <table width="600" border="0">
    <tr>
      <td>Stylesheet: </td>
      <td><select name="stylesheet" id="stylesheet" style="width:300px;">
    ...
    <tr>
      <td>Generate: </td>
      <td><select name="generate" id="generate" style="width:300px;">
